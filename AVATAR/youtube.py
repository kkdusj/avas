...
async def downloaded(client: Client, message):
 if len(message.command) == 1:
  if message.chat.type == enums.ChatType.PRIVATE :
   ask = await client.ask(message.chat.id, "عايز تنزل اي")
   query = ask.text
   m = await ask.reply_text("**جاري البحث انتظر  ✘**")
  else:
   ask = await client.ask(message.chat.id, "عايز تنزل اي", filters=filters.user(message.from_user.id), reply_to_message_id=message.id, timeout=8)
   query = ask.text
   m = await ask.reply_text("**جاري البحث انتظر  ✘**")
 else:
  query = message.text.split(None, 1)[1]
  m = await message.reply_text("**جاري البحث انتظر  ✘**")
  if message.command[0] in ["/song", "نزل", "تنزيل"]:
    ...
    try:
        ...
        rep = f"• uploader @TR_E2S_ON_MY_MOoN"
